    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {(stock ?? []).map((s) => {
          ...
          const low = coverage !== null && coverage !== undefined && coverage <= (s.low_stock_lead_time_days ?? 7) + (s.low_stock_safety_buffer_days ?? 3)
          return (
            <Card key={s.feed_product_id}>
              <CardContent className="space-y-1 py-3">
                ...
                <div className="flex justify-between text-sm text-muted-foreground">
                  <span>{(s.balance_sacks ?? 0).toFixed(1)} sak ({(s.balance_kg ?? 0).toFixed(0)} kg)</span>
